...
def init_model(checkpoint_path, device, precision, compile=False):
    ...
    # PATCH (utilisateur / 16GB VRAM): ne pas forcer le dtype. `from_pretrained` charge déjà
    # les poids avec le bon dtype via load_state_dict(assign=True). Forcer
    # `dtype=precision` (bf16) reconvertissait les poids int8 en bf16, annulant la
    # quantification int8 et provoquant un OOM sur GPU 16GB.
...
def launch_thread_safe_queue(
    ...
    def worker():
        ...
        with torch.device(device):
            # PATCH (utilisateur / 16GB VRAM): ne pas pré-allouer le cache KV pour 32768 tokens
            # (≈ 4.5 GB). La WebUI limite max_new_tokens à 2048 ; 16384 laisse une
            # large marge pour l'audio de référence + la génération.
            model.setup_caches(
                max_batch_size=1,
                max_seq_len=min(model.config.max_seq_len, 16384),
                dtype=next(model.parameters()).dtype,
            )
        init_event.set()
