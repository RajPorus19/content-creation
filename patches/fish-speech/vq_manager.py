...
class VQManager:
    ...
    def encode_reference(self, reference_audio, enable_reference_audio):
        if enable_reference_audio and reference_audio is not None:
            ...
            # PATCH (utilisateur / 16GB VRAM): le codec est chargé en bf16 (voir
            # patches/fish-speech/dac_inference.py). `load_audio` renvoie du
            # float32 → il faut caster sur le dtype du modèle avant encode(),
            # sinon torch lève "Input type (float) and bias type (c10::BFloat16)
            # should be the same".
            dtype = next(self.decoder_model.parameters()).dtype
            audios = torch.from_numpy(reference_audio_content).to(
                device=self.decoder_model.device, dtype=dtype
            )[None, None, :]
